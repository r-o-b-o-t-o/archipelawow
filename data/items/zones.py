...
class ZonesContainer(ItemContainer):
    ...
    def get_slot_data(self, world: "World"):
        return [[item.id, item.zone_id, item.icon, item.gossip_menu, item.teleport.to_list(), item.keys] for item in self.build_pool(world)]
...
DUN_MOROGH = Zone("Dun Morogh", "Zone_DunMorogh", 1, L01, "", TP(0, -6240, 331, 383, 0), side=A)
DUSKWOOD = Zone("Duskwood", "Zone_Duskwood", 10, L20, "", TP(0, -10864.93, 599.2, 31.35, 4.78), side=A)
EASTERN_PLAGUELANDS = Zone("Eastern Plaguelands", "Zone_EasternPlaguelands", 139, L55, L55, TP(0, 1924.59, -2632.02, 60.87, 4.72))
ELWYNN_FOREST = Zone("Elwynn Forest", "Zone_ElwynnForest", 12, L01, "", TP(0, -8950, -132.5, 84.5, 0.2), side=A)
...
SWAMP_OF_SORROWS = Zone("Swamp of Sorrows", "Zone_SwampSorrows_01", 8, "", L35, TP(0, -10515.52, -2377, 80.42, 5.79), side=H)
...
WESTFALL = Zone("Westfall", "Zone_WestFall_01", 40, L10, "", TP(0, -9817.96, 852.6, 26.96, 2.15), side=A)
...
THE_BARRENS = Zone("The Barrens", "Zone_Barrens_01", 17, "", L10, TP(1, 315.04, -3742.17, 35.11, 1.62), side=H)
...
BLACKROCK_DEPTHS = Zone("Blackrock Depths", "Boss_EmperorDagranThaurissan", 1584, L50, L50, TP(230, 458.3, 26.5, -69.7, 4.95), keys=[11000, 11140])
BLACKROCK_SPIRE = Zone("Blackrock Spire", "Boss_Overlord_Wyrmthalak", 1583, L55, L55, TP(229, 78.2, -225, 50.8, 4.73))
DIRE_MAUL = Zone("Dire Maul", "Ability_Warrior_DecisiveStrike", 2557, L55, L55, TP(429, 255.2, -16.1, -1.6, 4.7), keys=[18249])
GNOMEREGAN = Zone("Gnomeregan", "Boss_Mekgineer_Thermaplugg ", 721, L25, L25, TP(90, -332.2, -2.3, -151.8, 2.77), keys=[6893])
...
SCARLET_MONASTERY = Zone("Scarlet Monastery", "INV_Helmet_52", 796, L30, L30, TP(189, 1689, 1053.5, 19.68, 0), keys=[7146])
